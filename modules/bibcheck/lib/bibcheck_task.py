...
class Tickets(object):
    ...
    def tickets_per_record(self):
        ...
        for record in self.records:
            for issue in record.issues:
                output[record].append((issue.nature, issue.msg, issue.rule))
        for record in output.iterkeys():
            msg = []
            for issue in output[record]:
                issue_nature, issue_msg, rule = issue
    ...
    def tickets_per_rule_per_record(self):
        ...
        for record in self.records:
            for issue in record.issues:
                output[(issue.rule, record)].append((issue.nature, issue.msg, issue.rule))
        for issue_rule, record, rule in output.iterkeys():
            msg = []
